...
class ConstrainedDecoder:
    def __init__(self, model: Small_LLM_Model, functions: List[Dict[str, Any]]):
        self.model = model
    ...
    def _build_and_filter_vocab(self, raw_vocab: Dict[str, int]) -> List[tuple[str, int]]:
        print(f"Décodage et filtrage du vocabulaire ({len(raw_vocab)} tokens initiaux)...")
        filtered_items = []
        ...
        print(f"Vocabulaire optimisé réduit à {len(filtered_items)} tokens utiles.")
    ...
    def is_valid_prefix(self, prefix: str) -> bool:
        ...
        if '"' not in rem:
            if any(c.isspace() for c in rem): 
                return False
            return any(fn.startswith(rem) for fn in self.allowed_function_names)
    ...
    def generate_function_call(self, prompt: str, max_tokens: int = 150) -> str:
        functions_context = json.dumps(self.functions, indent=2)
        
        formatted_prompt = (
            f"System: You are an expert AI assistant. You must output a JSON function call.\n"
            f"Here are the available functions and their JSON schemas:\n{functions_context}\n\n"
            f"User: {prompt}\n"
            f"Assistant: {{"
        )
